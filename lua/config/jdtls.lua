...
local function get_bundles()
	...
	-- Obtain the full path to the directory whwere Mason has downloaded the Java Debug Adapter binaries
...
local function get_workspace()
	...
	-- Determine the local project name
	local project_name = vim.fn.fnamemodify(vim.fn.getcwd(), ":p:h:t")
	-- Create the workspace directory by concatenating the designated worksapce path and the project name
...
local function setup_jdtls()
	-- Get access to the jdtls plugin and all its functionality
	local jdtls = require("jdtls")

	-- Get the paths of the jdtls jar, operating specific configuration directory, and lombok jar
	...
	local root_dir = jdtls.setup.find_root({ ".git", "mvm", "gradlew", "pom.xml", "build.gradle" })
	...
	local settings = {
		java = {
			...
			format = {
				enabled = true,
				settings = {
					-- Use the Google guide for code formatting
					url = vim.vn.stdpath("config") .. "/lang_servers/intellij-java-google-style.xml",
					profile = "GoogleStyle",
				},
			},
			...
			-- Setup automatically package import organization on file save
			saveActions = {
			...
			sources = {
				-- How many  classes from a specific package should be imported befor automatic combine them all into one import
				organizeImports = {
					startThreshold = 9999,
	...
	-- Create a table called init_options to pass the bundles with debug and testing jar,
	-- along with the extended client capabilities to the start or attach function of JDTLS
	local init_options = {
		bundles = bundles,
	...
	local on_attach = function(_, buffer)
		...
		-- Find the main method(s) of the application so the debug adapter can successfully startup the application
		-- Sometimes this will randomly fail if langauge server take too long to startup for the project, if a ClassDefNotFoundException occurs when running
		-- the debug tool, attempt to run the debug tool while in the main class of the application or restart the neovim instance
		-- Unfortantely I have not found an elegant way to ensure this works 100% of the time
		require("jdtls.dap").setup_dap_main_class_configs()
		-- Enable 'jdtls_setup'.add_commands()
